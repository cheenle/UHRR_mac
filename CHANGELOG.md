...
#### 核心改进
将 ATR-1000 WebSocket 连接移到 Web Worker，进一步消除主线程阻塞。
...
```
旧架构:
主线程: ScriptProcessorNode (阻塞) + WebSocket 消息处理 (阻塞)

新架构:
主线程: 空闲 → 接收 Worker 数据 → DOM 更新
Worklet 线程: 音频处理
Worker 线程: ATR-1000 WebSocket + JSON 解析 + 心跳
```

#### 双重隔离
- **AudioWorklet**: 音频处理隔离
- **Web Worker**: ATR-1000 通信隔离
- **主线程**: 仅处理 DOM 更新
