...
check_system_resources() {
    ...
    # 磁盘空间
    DISK_USAGE=$(df -h "$MRRC_DIR" | tail -1 | awk '{print $5}')
...
check_logs_status() {
    ...
    LOG_FILES=(
        "$MRRC_DIR/mrrc_service.log"
        "$MRRC_DIR/mrrc_service_error.log"
        "$MRRC_DIR/mrrc_debug.log"
        "$MRRC_DIR/mrrc.log"
    )
...
check_errors_warnings() {
    ...
    # 检查服务错误日志
    if [ -f "$MRRC_DIR/mrrc_service_error.log" ]; then
        ERRORS=$(grep -i -e "error" -e "failed" -e "exception" "$MRRC_DIR/mrrc_service_error.log" | tail -5)
        if [ -n "$ERRORS" ]; then
    ...
    # 检查调试日志
    if [ -f "$MRRC_DIR/mrrc_debug.log" ]; then
        WARNINGS=$(grep -i "warning" "$MRRC_DIR/mrrc_debug.log" | tail -5)
        if [ -n "$WARNINGS" ]; then
        ...
        RECENT_ERRORS=$(grep -i -e "error" -e "failed" "$MRRC_DIR/mrrc_debug.log" | tail -5)
...
generate_health_report() {
    ...
    # 检查日志文件大小
    if [ -f "$MRRC_DIR/mrrc_service.log" ]; then
        LOG_SIZE=$(stat -f%z "$MRRC_DIR/mrrc_service.log")
