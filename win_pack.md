...
4. 把 `vendor\*\windows\bin\x64` 加入进程 `PATH`。
5. 启动 `MRRC-Server.exe <MRRC.conf>`。
6. 等待 HTTPS 端口响应，自动打开浏览器。

## 5. 注意事项

- MRRC 通过 `rigctld` 控制电台。Windows 用户需要自行运行 Hamlib 提供的 `rigctld.exe`，例如：
  ```powershell
  rigctld.exe -m 3060 -r COM3 -s 4800 -C stop_bits=2 -T 127.0.0.1 -t 4532
  ```
