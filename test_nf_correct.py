...
def process_wav_with_notch(input_file, output_file, notch_freq=1475, notch_width=150):
    """使用WDSP NF处理WAV文件"""
    ...
    # 添加额外的陷波点来覆盖边带
    extra_notches = [
        (notch_freq - 10, 80),  # 1465Hz
        (notch_freq + 10, 80),  # 1485Hz
    ]
    for freq, width in extra_notches:
        print(f"添加边带陷波点: {freq}Hz, 宽度: {width}Hz")
        processor.add_notch(freq, width)
    ...
    print("\n✅ 处理完成！")
    print(f"   陷波频率: {notch_freq}Hz (主频)")
    print(f"   陷波宽度: {notch_width}Hz")
    print(f"   额外陷波: 1465Hz, 1485Hz (覆盖边带)")

if __name__ == "__main__":
    input_file = "/Users/cheenle/UHRR/MRRC/recordings/07050kHz_20260402_070548.wav"
    output_file = "/Users/cheenle/UHRR/MRRC/recordings/07050kHz_20260402_070548_nf_correct.wav"
    
    try:
        process_wav_with_notch(input_file, output_file, notch_freq=1475, notch_width=150)
