...
class ModernHamInterface {
    ...
    initializeEventListeners() {
        ...
        document.getElementById('div-conf').addEventListener('click', (e) => {
            e.preventDefault();
            window.open('/CONFIG', '_MRRCconfig');
        });
        
        document.getElementById('div-panfft').addEventListener('click', (e) => {
            e.preventDefault();
            window.open('/panfft.html', 'MRRCpanfft', 'width=1000, height=1000, menubar=no, toolbar=no, location=no, resizable=yes, scrollbars=no, status=no, dependent=yes');
        });
