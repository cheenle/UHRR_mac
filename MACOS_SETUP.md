...
2. Update the `MRRC.conf` file with the correct device names for your system.

### 4. Configure Serial Port (for radio control)

Update the `MRRC.conf` file with the correct serial port for your radio:
...
1. **Audio device not found**: Check device names in `MRRC.conf`
