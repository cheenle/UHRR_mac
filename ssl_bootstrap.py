...
A fresh Windows install has no certificates, and `MRRC` only enables SSL when
cert+key exist — so the launcher generates a throwaway self-signed certificate
on first run (10-year validity, SANs covering localhost / hostname / LAN IPs)
and reuses it afterwards.  This is a convenience certificate for LAN use:
browsers will show an "untrusted" warning the user accepts once.  Real
deployments can point `certfile` / `keyfile` in `MRRC.conf` at proper
...
def ensure_self_signed(cert_dir: Path):
    ...
    Returns None when the cryptography package is unavailable or generation
    fails — the caller then must either skip HTTPS or abort startup.
    """
