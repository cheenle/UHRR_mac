...
>   时间水印抖动缓冲、软膝限幅、WDSP 恢复 48k（消"水音"）+ NR2 AE 调参
> - 🎙️ **TX 高音质**：48kHz 全带宽、64kbps CBR、标签帧、服务端软膝、保真收紧
>   （前置增益 0dB、压缩器透明、EQ 默认调平）
> - 🧹 **清理与发布**：移除嵌套 git 仓库与私钥、脚本缓存破（版本号+no-cache）、restart.sh
>
> 🎉 **V5.7.1 更新亮点**：
> - 🎙️ **RX/TX 链路第二轮优化**：TX 采集迁移 AudioWorklet、PTT 尾音/预热帧修复、RX 20ms 对齐采集
> - 📡 **ATR-1000 幽灵功率修复**：TX 结束即清零功率/SWR 缓存，广播线程安全修复
> - 🎛️ **IC-M710 AGC/RF 增益控制**：快捷行新增 AGC 开关 + RF 9-1 档
> - 🗑️ **FT8/CW 功能整体移除**
>
...
### 音频系统
- **双向音频**：TX 端 48kHz Opus 编码（AudioWorklet 采集），RX 端 16kHz 低抖动播放（AudioWorklet 水印缓冲）
...
- **Tune 联动调谐**：长按 Tune 发射 1kHz 单音时，若 SWR > 1.6 自动触发 ATR-1000 完整调谐；若调谐后驻波降低，更新当前频率记忆参数；若未降低，恢复原 LC/CL、电感、电容参数
...
| RX 线格式 | 1 字节标签帧（0x00=PCM / 0x01=Opus） |
