...
- **Core Components**:
  ...
  - User authentication system in `UHRR_users.db`
...
### Configuration
- **Main Configuration**: `UHRR.conf` - Contains server, audio, hamlib, and panadapter settings
- **Web Interface**: Access `/CONFIG` endpoint for web-based configuration
- **User Authentication**: `UHRR_users.db` SQLite database
...
### Critical Files
- `UHRR`: Main server executable with WebSocket handlers and radio control
- `UHRR.conf`: Configuration file with multiple sections (SERVER, CTRL, AUDIO, HAMLIB, PANADAPTER)
