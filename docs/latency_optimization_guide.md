...
V5.3 引入了 7 项 RX/TX 音频优化，进一步改善延迟与质量：

| 优化项 | 类型 | 延迟影响 |
|--------|------|----------|
| Pre-AGC Bypass | RX | 绕过双增益级联，减少 RX 处理路径延迟 |
| 录音抗混叠滤波器 | RX | 3 样本滑动平均，改善录音质量 |
| RX Opus 参数调优 | RX | `complexity=8, bitrate=28kbps`，质量与 CPU 平衡 |
| 前端 TX Opus 默认值 | TX | 编码器构造函数中集成优化参数，消除初始化错配 |
| TX 电平归一化 | TX | 非对称增益平滑（快起慢放），防止削波 |
| 自适应 Opus 码率 | TX | 根据队列深度动态调整码率，减轻拥塞延迟 |
| 前端 FEC/DTX | TX | 前向纠错 + 不连续传输，降低丢包影响 |

**关键延迟优化**：Pre-AGC Bypass 避免 WDSP AGC 与预 AGC 双增益处理，减少 RX 路径计算开销；自适应 Opus 码率在队列堆积时自动降码率，减轻拥塞导致的缓冲延迟。
