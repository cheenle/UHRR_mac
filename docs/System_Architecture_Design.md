...
### 1.1 系统定位
**Mobile Remote Radio Control (MRRC)** 是一款专为移动端优化的业余电台远程控制系统。设计理念为"Mobile First, Radio Anywhere"，让业余无线电爱好者可以随时随地通过移动设备或桌面浏览器远程控制电台设备。

**随时随地，畅享业余无线电。**

### 1.2 核心价值
- **📱 移动优先**: 专为触摸屏优化，支持单手操作，PWA离线访问
- **🌍 随时随地**: 互联网覆盖处即可操控您的电台
...
- **Web界面**: 基于HTML5的响应式界面
- **实时显示**: 频谱瀑布图、FFT显示、S表
- **控制面板**: 频率调节（优化布局：上加下减）、模式选择、音量控制
- **状态指示**: 连接状态、PTT状态、音频电平
- **功率显示**: ATR-1000实时功率/SWR
...
ATR-1000功率流:
ATR-1000设备 → 独立代理(atr1000_proxy.py) → Unix Socket → MRRC桥接 → WebSocket(/WSATR1000) → 移动端显示
...
#### 4.1.3 ATR-1000客户端模块 (`mobile_modern.js`)
- **连接管理**: 页面加载时预先连接，保持预热状态
- **数据接收**: 功率、SWR、继电器状态实时更新
- **天调存储**: 频率-参数自动匹配和保存
- **显示更新**: 实时功率条、SWR指示器
- **双重时间保护**: 确保sync请求最小间隔500ms

#### 4.1.4 WebSocket客户端
- **连接管理**: 自动重连和状态监控
- **消息路由**: 控制、音频TX、音频RX、频谱数据、ATR-1000分离处理
- **错误处理**: 连接丢失时的优雅降级
...
#### 4.2.4 ATR-1000桥接器 (`MRRC`)
- **WebSocket端点**: `/WSATR1000`
- **Unix Socket客户端**: 连接独立代理程序
- **线程安全**: IOLoop.add_callback()跨线程通信
- **批量广播**: 50ms批次收集，广播最新数据
- **实时转发**: 功率、SWR、继电器状态数据

#### 4.2.5 ATR-1000独立代理 (`atr1000_proxy.py`)
- **WebSocket服务端**: 连接ATR-1000设备
- **Unix Socket服务端**: 接收MRRC连接
- **协议解析**: SCMD_METER_STATUS、SCMD_RELAY_STATUS
- **SYNC机制**: TX期间500ms，空闲2s预热
- **自动重连**: 设备断开后自动重连

#### 4.2.6 天调存储模块 (`atr1000_tuner.py`)
...
#### 4.3.2 ATR-1000功率计/天调
- **接口**: WebSocket (默认192.168.1.63:60001)
- **协议**: 二进制协议，SCMD命令集
- **数据类型**: 功率(0-200W)、SWR(1.0-9.99)、继电器状态
- **控制功能**: TUNE天调启动、参数读取
...
- s:                                 # 停止TX
- <binary_audio_data>               # Int16编码音频数据

服务器处理:
- 解码并输出到电台音频设备
- PTT超时保护（计数法：连续10次×200ms未收到音频帧才熄灭PTT）
```

#### 5.1.3 音频RX通道 (WSaudioRX)
```
服务器到客户端:
- <binary_audio_data>    # Int16 PCM音频数据
...
#### 5.1.5 ATR-1000功率监控通道 (WSATR1000)
```
客户端到服务器:
- {"action": "start"}    # 开始数据流（TX开始时发送）
- {"action": "stop"}     # 停止数据流（TX结束时发送）

服务器到客户端:
- {"type": "atr1000_meter", "power": 100, "swr": 1.25, "vforward": 70.7, "vreflected": 7.07, "relay_status": {"lc": "LC", "inductance": 5, "capacitance": 3}}

数据格式说明:
- power: 前向功率 (瓦特，0-200W)
- swr: 驻波比 (无单位，1.0-9.99)
- vforward: 前向电压
- vreflected: 反射电压
- relay_status: 继电器状态
  - lc: LC或CL组合
...
- TX结束后保持连接（预热状态）
- SYNC预热：有客户端时每2秒发送
- TX期间：每500ms发送SYNC确保数据流
- 双重时间保护：确保sync最小间隔500ms

客户端处理:
- 实时显示功率和SWR数据
- 根据SWR值更新显示颜色（绿色<1.5，黄色<2.0，红色>2.0）
- 天调参数智能存储和匹配
```
...
```
Mobile Remote Radio Control (MRRC) - 移动端业余电台远程控制系统
...
| v3.0.0 | 2025-01 | 移动端优化，AAC编码，TCI协议 | 生产就绪 |
| v4.0.0 | 2026-03 | TX→RX延迟优化，架构精简，V4里程碑 | 生产就绪 |
| v4.1.0 | 2026-03 | 项目重命名MRRC，双语文档 | 生产就绪 |
| v4.2.0 | 2026-03 | TX EQ均衡器，短波语音优化 | 生产就绪 |
| v4.3.0 | 2026-03 | ATR-1000架构分离，独立代理 | 生产就绪 |
| v4.4.0 | 2026-03-05 | ATR-1000实时显示重大修复 | 生产就绪 |
| v4.5.4 | 2026-03-06 | ATR-1000实时功率显示稳定版 | 生产就绪 |
