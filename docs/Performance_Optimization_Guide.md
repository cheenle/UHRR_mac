...
## 11. V5.3 音频优化 (2026-05-20)

### 11.1 新增优化总览

V5.3 引入了 7 项 RX/TX 音频优化，进一步提升延迟、质量和鲁棒性：

| 优化项 | 类型 | 说明 |
|--------|------|------|
| Pre-AGC 旁路 | RX | WDSP AGC 激活时旁路前级 AGC，避免双重增益控制 |
| 录音抗混叠滤波 | RX | 3 采样点移动平均，抑制带外噪声混叠 |
| RX Opus 参数调优 | RX | `configure_for_voip(bitrate=28000, complexity=8, fec=True, packet_loss_perc=15, dtx=True)` |
| TX Opus 前端默认值 | TX | `opus_codec.js` 构造函数中设置优化的 Opus 编码器默认参数 |
| TX 电平归一化 | TX | 非对称增益平滑（快起慢放），防止削波 |
| 自适应 Opus 码率 | TX | 根据队列深度动态调整 Opus 编码码率 |
| 前端 FEC/DTX | TX | 启用前向纠错与间断传输，降低丢包影响 |

### 11.2 Pre-AGC 旁路

当 WDSP AGC 激活时，自动旁路前级 AGC 处理，避免两级增益串扰导致的音频失真或 pumping 效应。

**位置**: `wdsp_wrapper.py` / `MRRC` RX 音频链

### 11.3 录音抗混叠滤波器

在录音路径上应用 3 采样点移动平均低通滤波器，抑制 ADC 采样产生的带外噪声混叠。

**位置**: `recorder.py` / 音频采集管道

### 11.4 RX Opus 参数调优

Opus 解码器重新配置，优先语音质量与抗丢包：

```python
configure_for_voip(
    bitrate=28000,       # 28 kbps（此前 20 kbps），提升音质
    complexity=8,        # 编码复杂度 8（此前 5），平衡 CPU 与质量
    fec=True,            # 启用前向纠错
    packet_loss_perc=15, # 预期 15% 丢包率，触发带内 FEC
    dtx=True             # 间断传输，静音帧不编码
)
```

**位置**: `MRRC` RX AudioHandler / `audio_interface.py`

### 11.5 TX Opus 前端默认值

前端 `opus_codec.js` 构造函数集成优化后的编码器参数，确保 TX 音频一建立即使用最佳配置。

**位置**: `www/opus_codec.js`

### 11.6 TX 电平归一化与非对称增益平滑

发射路径应用非对称增益平滑（attack fast, release slow），实时归一化音频电平防止削波，同时避免增益快速回落造成的听觉不适。

**位置**: `MRRC` TX 音频处理 / `audio_interface.py`

### 11.7 自适应 Opus 码率

根据后端 TX 音频队列深度动态调节 Opus 编码码率：队列堆积时降低码率缓解拥塞，队列空闲时提升码率保证音质。

**位置**: `MRRC` TX AudioHandler

### 11.8 前端 FEC/DTX

前端 WebSocket 发送路径启用 Opus FEC（带内前向纠错）和 DTX（间断传输），降低网络丢包对语音连续性的影响，同时节省静音期间带宽。

**位置**: `www/opus_codec.js`

---

*本性能优化指南基于 MRRC V5.3 稳定版本验证。*
*更新时间: 2026-05-20*