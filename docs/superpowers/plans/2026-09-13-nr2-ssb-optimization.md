...
## as-built 附记（2026-09-17 水声闭环，第二轮）

用户反馈"水声还是很大" → 搭了可复跑的闭环 `dev_tools/nr2_loop_opt.py`（真实录音 → 生产链路离线复现 →
无参考指标 → 分阶段搜索 → A/B 样本）。本轮三个测量陷阱与修法（都写进了工具 docstring）：

1. **实时性**：wdsp 是多线程实时引擎，喂数快于实时 → 内部队列饥饿/丢块、结果不可复现 → speed=1.0 + 9s 片段 + 多进程并行；
2. **AGC 污染**：AGC 的静态增益与泵动把"输出/输入"型度量全部污染（14dB 的钳位差异被压成 3dB、
   语音衰减 100% 误报）→ 归因全程 AGC=OFF，并用 NR-OFF 基线测出静态增益 G 做逐项校正；
3. **对齐**：输出相对输入有 ~150ms 处理时延（lag≈25-28 帧）→ 带内包络互相关对齐 + 饥饿(-2)直通帧掩蔽；
   音乐噪声指标按帧行归一（剥离 QSB 共模衰落）。

实测结论（2 条 40m 录音、指标=闪烁比/吃字率/深度/包络相关）：

| 配置 | 深度 | 吃字 | 音乐噪声 | 结论 |
|---|---|---|---|---|
| L2 旧 (-12) | 4.9dB | 26.7% | ×1.22 | 深度小、吃字重 ← 用户听到的"水声" |
| **-9（新 L2）** | 4.1dB | **8.2%** | ×1.15 | 深度只让 0.8dB，吃字降 2/3 |
| -16（旧 L3） | 5.4dB | 32% | ×1.32 | 深度不再增长 |
| -20（旧 L4） | 5.7dB | 33% | ×1.41 | 同上 |

→ 等级表整体下移一档：`MAX_ATTEN_DB = {1:-6, 2:-9, 3:-12, 4:-16}`（随 6.1.17 热修下发）。
dry 混合使深度腰斩（不划算）；psi/zeta 在真实素材上几乎无感（AE 少触发）。
钳位本身经新旧 dylib A/B 验证均生效（早期"钳位失效"判断是 AGC 污染造成的误诊）。
