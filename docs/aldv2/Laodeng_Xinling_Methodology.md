...
> **从定性到可证伪**:上表八类是"软信号",容易自我安慰。真要证明方法有效,把其中三类换成可量化的硬指标——接手探索时间(对应 Harness 完整度)、同类事故复发次数(对应记忆保真度)、相似功能边际工时(对应 Product 杠杆)。能画出下降曲线才算数。详见 `Aladdin_V2_Critical_Review.md` 与 `Aladdin_V2_Business_Toolkit.md` 工具 3。
...
旧版列了十一条。实操中没人背得下。全部收敛到三层循环,只留七条——每层两条,加一条贯穿。

**L1 定界(老灯 + 现场)**
1. 老兵判断定义方向、边界、风险和质量底线;现场事实是最高证据源,判断必须接受现实校验。
2. 边界先于速度写下来:没有边界,agent 只是高速猜测。

**L2 提速(新灵 + FDE)**
3. 新灵只在边界内提速,负责放大工程能力,不替代工程判断。
4. Delta 代码可以粗糙,但学习必须清晰;先验最大不确定性,超 5 轮不收敛要换方案。

**L3 沉淀(SDD + Harness + 杠杆)**
5. SDD 是架构脊柱不是瀑布枷锁;harness 是 agent 安全工作的轨道;postmortem 是未来速度。
6. 每圈必须把学习变成杠杆——这次成果是否让下一次更便宜,是唯一终极标准。

**贯穿**
7. 老灯不灭,新灵不飘;老兵定边界,现场给证据,智能提速度,架构保长期。但方法论也要标注自己的射程边界,出了射程自己补装备(见批判分析与业务工具包)。

---

文档版本: V2.0  
更新记录:
- V2.0 (2026-06):七层结构精炼为**三层一循环**(2.1–2.7 降为三层内部角色);最终原则收敛重写;补入批判分析与业务工具包的交叉引用。
- V1.0:初版,七层结构 + 标准流程 + agentic 规则 + 度量 + 反模式 + 模板。

基于材料: `FDE.md`, `docs/FDE_Practice_Guide.md`, `SDD/`, `DESIGN/MRRC_SDD.md`, `docs/superpowers/`, `AGENTS.md`, MRRC 运行和诊断脚本。  
配套文档: `Aladdin_V2_Methodology.md`(总纲), `Aladdin_V2_Critical_Review.md`(批判), `Aladdin_V2_Business_Toolkit.md`(实操)。
