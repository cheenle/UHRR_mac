...
度量按三层组织。**先看硬指标,定性信号只是补充**——只有定性指标的方法论容易"自我安慰":团队可以宣称健康而无外部可证伪依据(见批判分析)。

| 层 | 硬指标(可证伪,优先) | 健康趋势 | 定性软信号 |
|----|----------------------|----------|------------|
| L1 定界 | 边界卡覆盖率(动手前有卡的任务占比) | 趋近 100% | 老兵判断进入 ADR/AGENTS/plan,而非只在脑中 |
| L2 提速 | 迭代收敛轮数 | 稳定在 3–5 轮 | 每个 cycle 有真实现场证据;Delta 结束能 demo |
| L3 沉淀 | ① 接手探索时间 ② 同类事故复发数 ③ 相似功能边际工时 | ①↓ ②→0 ③↓ | NFR 有验证方法;harness 让 agent 不必重新探索入口 |

三个 L3 硬指标是方法论是否真产生杠杆的最终证据:**画不出下降曲线,Product 杠杆就只是口号**。采集模板见 `Aladdin_V2_Business_Toolkit.md` 工具 3。

警戒信号:连续两个 cycle 无抽象产出;连续一版未更新 SDD/ADR;agent 改动前必须重新探索大量基础事实;同类事故重复发生。
...
全部原则收敛到三层循环。背不下七条,就背这一句:**没有边界不动手,每圈必留资产。**

**L1 定界**
1. 老兵判断定方向、边界、风险和质量底线;现场事实是最高证据源。
2. 边界必须先于速度被写下来——没有边界的 vibe coding 只是高速猜测。

**L2 提速**
3. 新灵只在边界内提速:先验最大不确定性,3-5 轮收敛,每轮能 demo。
4. Delta 代码是消耗品,允许粗糙,但学习必须清晰。

**L3 沉淀**
5. 每圈至少留一个可复用资产(SDD/ADR/harness/postmortem),否则不算完成。
6. Product 杠杆是唯一终极标准:这次是否让下一次更便宜。

**贯穿三层**
7. 标注射程边界——方法论也要对自己用,出了射程自己补装备(见批判分析与业务工具包)。

---

文档版本: V2.0  
更新记录:
- V2.0 (2026-06):七层模型精炼为**三层一循环**(定界→提速→沉淀);最终原则按三层重写;新增批判分析与业务工具包两份配套文档。
- V1.0:初版,七层模型 + FDE/SDD/Harness/杠杆。

核心模型: `docs/aldv2/Laodeng_Xinling_Methodology.md`  
批判分析: `docs/aldv2/Aladdin_V2_Critical_Review.md`  
业务工具包: `docs/aldv2/Aladdin_V2_Business_Toolkit.md`  
