...
### 2.0 Opus 编码优化 (V5.0.0) ✅

**Opus 比特率提升**: 20kbps → 28kbps
- **位置**: `audio_interface.py:521`
- **实现**: `bitrate=28000` 替代默认 20000
- **效果**: 提升语音保真度，特别在弱信号恢复时更清晰

**Opus 编码复杂度提升**: 5 → 8
- **位置**: `audio_interface.py:521`
- **实现**: `complexity=8` 替代默认 5
- **效果**: 更优的比特分配，同等码率下更高的主观音质

**AGC 预放绕过优化**
- **位置**: `audio_interface.py` (音频处理链路)
- **实现**: 电平充足时自动跳过前置 AGC 预放大
- **效果**: 避免双重增益放大引入的失真和噪声
