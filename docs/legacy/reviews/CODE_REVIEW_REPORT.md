...
**V5.3 缓解措施**:
- Opus 编码复杂度从 10 降至 5（V5.3 调整为 8），在质量与 CPU 负载间取得平衡
- 编码器在 `opus_codec.js` 构造函数中通过 `_opus_encoder_ctl` 设置优化默认值（complexity=8, bitrate=28kbps, VBR=ON, FEC=ON(15%), DTX=ON, HPF=OFF），消除 JS 属性赋值失效的 bug
- 自适应码率机制根据后台队列深度动态调整，降低拥塞时的主线程压力
