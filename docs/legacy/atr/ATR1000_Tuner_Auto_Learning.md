...
- **实时监测**：发射时实时显示功率和 SWR
- **Tune 联动保护**：长按 Tune 发射 1kHz 单音时，若 SWR 大于 1.6，会自动触发 ATR-1000 完整调谐；若调谐后 SWR 改善，则更新记忆参数，否则恢复原始 LC/CL、电感、电容参数
...
移动端和主界面的 `Tune` 按钮仍保持原有语义：按下开始发射 1kHz 单音，松开停止发射。新增联动只在 Tune 已经进入发射状态后运行，并满足以下条件才触发：

1. ATR-1000 WebSocket 已连接
2. Tune 发射期间已经收到有效功率/SWR 数据
3. 初始 SWR 大于 1.6
...
按下 Tune
    │
    ▼
MRRC 发射 1kHz 单音并通知 ATR-1000 进入 TX 数据刷新
    │
    ▼
采样初始 SWR 和当前继电器参数（LC/CL、IND、CAP）
    │
    ▼
SWR > 1.6?
    │
    ├─ 否 ─→ 保持原 Tune 发射流程，不启动 ATR-1000 调谐
    │
    └─ 是 ─→ 发送 ATR-1000 完整调谐命令（mode=2）
              │
              ▼
          等待调谐状态结束或参数/SWR 稳定
              │
          ...
          调谐后 SWR < 初始 SWR?
              │
              ├─ 是 ─→ 保留新天调参数，并更新当前频率的记忆参数
              │
              └─ 否 ─→ 恢复 Tune 前的 LC/CL、IND、CAP
```

### 保护行为

- 用户松开 Tune 时会取消联动流程，避免在无 RF 输出时继续根据残留数据判断。
- 如果设备上报 `SCMD_TUNE_STATUS`，前端优先使用调谐状态判断完成；如果设备没有明确上报完成状态，则使用 SWR/继电器参数稳定时间作为兜底。
- 代理层为调谐状态设置 45 秒超时保护，避免异常情况下前端长期认为设备仍在调谐。
- “未改善”的判断使用严格比较：调谐后 SWR 必须低于初始 SWR，否则恢复原参数。
- 如果完整调谐后的 SWR 低于初始值，前端会静默更新本地 `atr1000_tuner_records`，并向 ATR-1000 代理发送 `learn` 命令，将新 LC/CL、IND、CAP 写入持久化学习记录。
...
**第 1 层：代理入口门槛**
- 按实测功率判定发射中（`power >= LEARN_MIN_POWER`，默认 3W）——V5.8.5 起不再依赖
  前端 `start` 信号（`is_tx`），面板直发/外部软件路径同样可学习
- 不在调谐流程中（`tuning = False`）
- 当前频率已知（`freq > 0`）

**第 2 层：忽略窗口（1.0 秒）**
- TX 开始 ≥ 1.0 秒后
- 继电器切换 ≥ 1.0 秒后
- 目的：过滤 TX 起始瞬态 SWR=1.00 假数据

**第 3 层：稳定窗口缓冲器（LearningBuffer）**
- 继电器参数与窗口基线一致
- 功率 ≥ 3W（`LEARN_MIN_POWER`）
- 1.0 ≤ SWR ≤ 1.8（`LEARN_SWR_MIN/MAX`）
- 窗口填满 4 个连续样本（`LEARN_WINDOW_SIZE`）
- 窗口内 SWR 波动 ≤ 0.08（`LEARN_SWR_STABILITY`）

**第 4 层：TunerStorage 写入校验**
- SWR 在 1.0 ~ 1.8 范围（二次校验）
- 电感/电容参数不全为零（排除无效配置）

**第 5 层：继电器参数覆盖（已有记录时）**
- 新 SWR < 历史 `swr_avg` 且 SWR ≥ 1.00 → 更新 sw/ind/cap
- 否则只更新 SWR 统计，保持原参数

**第 6 层：分级去重冷却（V5.6.4）**
- 相同 `(freq, sw, ind, cap)` 组合：SWR 恶化 >0.01 → 跳过（旧参数更好）；
  SWR 基本不变（±0.01）且距上次 <5s → 跳过；SWR 改善 ≥0.02 → 立即学
- 防止每个 METER 采样都触发原子写盘
...
- **V5.8.5** (2026-08-30): 自动学习修复
  - 修复 `dispatch()` 嵌套函数未声明 `global is_tx` 的回归（V5.7.1 引入）——
    `start`/`stop` 只改写局部变量，模块级 `is_tx` 恒为 False，学习与 TX 轮询静默失效
  - 学习入口由 `is_tx`（前端 start 信号）改为按实测功率判定发射
    （`power >= LEARN_MIN_POWER`，默认 3W），覆盖面板直发/外部软件路径
  - 同步修复 `dispatch` 内 `_swr_high_since` 赋值（SWR 守卫复位此前同样失效）

- **V5.6.3** (2026-06-09): 学习去重冷却机制
  - 新增第 6 层去重：相同 `(freq,sw,ind,cap)` + SWR 变化 <0.03 且距上次 <2s → 跳过 `tuner.learn()`
  - 修复稳定窗口满后每个 METER 采样都触发原子写盘的问题（~8-10 次/秒 → ~1 次/2秒）
  - 消除 `sample_count` 虚高和 `swr_avg` 被冗余调用污染的缺陷
  - 修复 `last_learn_freq` 只抑制日志不抑制实际 `tuner.learn()` 调用的 bug

- **V5.6.0** (2026-05-XX): 稳定窗口学习缓冲器
  - 新增 `LearningBuffer` 类，防 TX 瞬态假 SWR 污染
  - 连续 4 样本 SWR 稳定（波动 ≤0.08）才提交学习
