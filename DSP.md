...
**版本**: V5.0.0 (2026-05-20)  
**更新**: Opus 码率提升至 28kbps, 编码复杂度优化至 8, 新增 AGC 预放绕过优化; 添加项目目录库路径支持，优化日志输出
...
**优化**: 新增 AGC 预放绕过优化——当输入音频电平充足时自动跳过前置 AGC 预放大环节，避免双重增益放大带来的失真，同时降低处理延迟。
...
WDSP 处理 (48kHz)
    ...
    └── AGC 自动增益
    ↓
Opus 编码 (16kHz/28kbps, complexity=8)
    ↓
WebSocket 传输
    ↓
前端解码播放
...
5. **Opus 配置**: 比特率从 20kbps 提升至 28kbps，编码复杂度从 5 提升至 8，在合理 CPU 开销下获得更佳语音保真度
6. **AGC 预放绕过**: 电平充足时跳过前置预放大，降低失真风险
...
| 版本 | 日期 | 变更 |
|------|------|------|
| V5.0.0 | 2026-05-20 | Opus 码率提升至 28kbps, 编码复杂度优化至 8, AGC 预放绕过优化 |
